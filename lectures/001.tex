...
\item eine reguläre $C^q$-Parametrisierung $\varphi:V\subset\mathbb{R}^d\rightarrow\mathbb{R}^n$ ($V$ ist offen) existiert, die homöomorph ist und in die Mannigfaltigkeit abbildet (also \  $\varphi(V)=U$).
\end{enumerate}
...
\tag{$\ast$}
\end{equation*}
Wir nennen $M=\{u\in D \ | \ f(u)=0\}$ die Niveaumenge von $f$\\
